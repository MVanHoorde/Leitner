...
/** Plafond accepté à la relecture d'une sauvegarde venant d'une autre version. */
const ITERATIONS_MAX_PBKDF2 = 5000000;
...
async function lireSauvegarde(texte, demanderMotDePasse) {
  ...
  let contenu = fichier;
  if (fichier.chiffre) {
    // Décodé une seule fois, avant la boucle : un fichier abîmé doit se distinguer
    // d'un mot de passe erroné, sinon la saisie serait redemandée indéfiniment.
    let sel; let iv; let ct;
    try {
      sel = depuisBase64(fichier.kdf.sel);
      iv = depuisBase64(fichier.iv);
      ct = depuisBase64(fichier.donnees);
    } catch {
      throw new Error('Cette sauvegarde chiffrée est incomplète ou abîmée.');
    }
    const iterations = fichier.kdf.iterations;
    // Borné : un nombre d'itérations aberrant fige l'appareil pendant la dérivation.
    if (!Number.isInteger(iterations) || iterations < 1 || iterations > ITERATIONS_MAX_PBKDF2) {
      throw new Error('Cette sauvegarde chiffrée est incomplète ou abîmée.');
    }
    for (;;) {
      const motDePasse = await demanderMotDePasse();
      if (motDePasse === null) return null;
      const cle = await Chiffrement.deriverCle(motDePasse, sel, iterations);
      try {
        contenu = await Chiffrement.dechiffrer(cle, { iv, ct });
        break;
      } catch {
